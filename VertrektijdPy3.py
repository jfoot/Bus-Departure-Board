# This software was produced by Jonathan Foot (c) 2024, all rights reserved.
# Project Website : https://departureboard.jonathanfoot.com
# Documentation   : https://jonathanfoot.com/Projects/DepartureBoard
# Description     : This program allows you to display a live bus departure board for Amsterdam.
# Python 3 Required.
...
requiredNamed.add_argument("-k","--APIKey", help="Your API Key, you can get your own at: https://www.vertrektijd.info/starten.html", type=str,required=True)
requiredNamed.add_argument("-s","--StopCode", help="The Stop Code for the specific station you wish to display.", type=str,required=True)
# Don't have a via message
parser.set_defaults(ReducedAnimations=True)
...
class LiveTime(object):
	...
	def __init__(self, Data, Index):
		self.ID =  str(Data['JourneyNumber'])
		self.Operator = str(Data['AgencyCode'])
		self.ServiceNumber = self.GetServiceNumber(Data, Index)
		self.Destination = str(Data['Destination'])
		self.SchArrival = str(Data['PlannedDeparture'])
		self.ExptArrival = str(Data['ExpectedDeparture'])
	...
	def GetDisplayTime(self):
		...
		Arrival = datetime.strptime(self.ExptArrival, '%Y-%m-%dT%H:%M:%S')
	...
	def GetServiceNumber(self, Data, Index):
		if Args.ServiceName == "1":
			return "%s.%s" % (Index + 1,str(Data['LineNumber'])) if Args.ShowIndex else str(Data['LineNumber'])
		elif Args.ServiceName == "2":
			return "%s.%s" % (Index + 1,str(Data['LineName'])) if Args.ShowIndex else str(Data['LineName'])
	...
	def GetData():
		...
		try:
			url = f"https://api.vertrektijd.info/departures/_stopcode/{Args.StopCode}/"
			headers = {
				'Accept': 'application/json',
				'Accept-Version': '1.5.0',
				'X-Vertrektijd-Client-Api-Key': Args.APIKey
			}
			request = Request(url, headers=headers)
			with urlopen(request) as conn:
				tempServices = json.loads(conn.read())
				for service in tempServices['BTMF'][0]['Departures']:
...
def Splash():
	if Args.SplashScreen:
		with canvas(device) as draw:
			...
			draw.multiline_text((45, 35), "Version : 1.0.VE -  By Jonathan Foot", font=ImageFont.truetype("%s/resources/Skinny.ttf" % (os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))),15), align="center")
